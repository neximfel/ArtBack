...
async function initDB() {
  ...
  if (count.c === 0) {
    ...
    // 🔧 ФИКС: Все 16 работ с гарантированно рабочими изображениями с Unsplash
    await run(`INSERT INTO artworks VALUES ('a1','demo1','Дракон над горами','Концепт-арт для игровой студии. Нарисовано в Photoshop за 8 часов.','digital','["фэнтези","концепт","цифра"]',142,1250,3500,'https://images.unsplash.com/photo-1577493340887-b7bfff550145?w=800&q=80','linear-gradient(135deg,#3b82f6,#1e3a8a)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a2','demo2','Рассвет в Альпах','Снято на Canon R5 с объективом 24-70mm. Золотой час в швейцарских Альпах.','photography','["пейзаж","горы","природа"]',98,890,2100,'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=800&q=80','linear-gradient(135deg,#059669,#065f46)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a3','demo3','Маленький волшебник','Иллюстрация для обложки детской книги о мальчике, который нашёл волшебную палочку.','illustration','["книга","персонаж","сказка"]',215,2100,5200,'https://images.unsplash.com/photo-1518182170546-0766aa6f6a56?w=800&q=80','linear-gradient(135deg,#d97706,#92400e)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a4','demo4','Киберпанк город','3D-сцена ночного города в стиле киберпанк. Создано в Blender.','3d','["киберпанк","город","научная фантастика"]',178,1560,4100,'https://images.unsplash.com/photo-1555617981-778dd1c43165?w=800&q=80','linear-gradient(135deg,#6366f1,#4338ca)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a5','demo1','Портрет эльфийки','Детальный портрет в стиле фэнтези. Акцент на глаза и волосы.','digital','["портрет","эльф","фэнтези"]',203,1890,4800,'https://images.unsplash.com/photo-1535295972055-1c762f4483e5?w=800&q=80','linear-gradient(135deg,#118ab2,#065f46)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a6','demo2','Туманный лес','Мистическая атмосфера утреннего тумана в осеннем лесу.','photography','["лес","туман","осень"]',134,1100,2800,'https://images.unsplash.com/photo-1511497584788-87676011196d?w=800&q=80','linear-gradient(135deg,#2d6a4f,#1e293b)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a7','demo3','Лесная фея','Иллюстрация для серии открыток. Фея, которая прячется среди цветов.','illustration','["фея","природа","открытка"]',189,1650,3900,'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=800&q=80','linear-gradient(135deg,#ec4899,#be185d)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a8','demo4','Космический корабль','3D-модель исследовательского корабля для инди-игры.','3d','["космос","корабль","игра"]',156,1320,3200,'https://images.unsplash.com/photo-1614728894747-a631e4be966b?w=800&q=80','linear-gradient(135deg,#7c3aed,#4c1d95)',CURRENT_TIMESTAMP)`);
    ...
    await run(`INSERT INTO artworks VALUES ('a11','demo3','Зимняя сказка','Иллюстрация новогодней книги. Девочка и её олень в зимнем лесу.','illustration','["зима","сказка","новый год"]',198,1780,4200,'https://images.unsplash.com/photo-1512474932049-78ac69ede12c?w=800&q=80','linear-gradient(135deg,#3b82f6,#1e40af)',CURRENT_TIMESTAMP)`);
    await run(`INSERT INTO artworks VALUES ('a12','demo4','Механический дракон','3D-модель стимпанк дракона для настольной игры.','3d','["стимпанк","дракон","механизм"]',172,1490,3700,'https://images.unsplash.com/photo-1599839575945-a9043f0a4d1f?w=800&q=80','linear-gradient(135deg,#dc2626,#991b1b)',CURRENT_TIMESTAMP)`);
    ...
    await run(`INSERT INTO artworks VALUES ('a14','demo1','Подводный мир','Концепт-арт подводной цивилизации. Атмосфера глубины.','digital','["подводный","океан","цивилизация"]',159,1380,3400,'https://images.unsplash.com/photo-1582967788606-a171f1080ca8?w=800&q=80','linear-gradient(135deg,#06b6d4,#0e7490)',CURRENT_TIMESTAMP)`);
